...
p_amt <- trade_sell %>% ggplot(aes(x=Date, y=gain, fill=Currency))+geom_col()+
  scale_y_continuous(labels=dollar, expand=expansion(add=c(0,100)))+
  labs(x="")

p_pc <- trade_sell %>% ggplot(aes(x=Date, y=roi, fill=Currency))+geom_col()+
