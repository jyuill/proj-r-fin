...
```{r}
## function to get latest price and cost data for table
price_acb <- function(curr_price_cost){
  ## get of the col with specific asset price
  cprice_col <- colnames(curr_price_cost)[2]
  ...
  curr_price_cost %>% filter(date==max(curr_price_cost$date)) %>% 
  select(date, acb, {{cprice_col}}) %>%
    mutate(diff=percent((.[[3]])/acb-1),
      {{cprice_col}} := currency(.[[3]]),
           acb=currency(acb),
           )
}
## print key info using price data and acb for symbol
price_acb(curr_price_cost) %>% kable() %>% 
...
```{r}
## print key info using price data and acb for symbol
price_acb(curr_price_cost) %>% kable() %>% kable_styling(full_width = FALSE)
```
