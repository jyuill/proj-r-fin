...
curr_trans %>% ggplot(aes(x=date_trans, y=acb))+geom_col()
curr_trans %>% ggplot(aes(x=date_trans, y=cumunits_calc))+geom_col()
...
curr_price_cost %>% ggplot(aes(x=date, y=`BTC-CAD.Close`))+geom_line(color=price_line)+
...
curr_price_cost %>% ggplot(aes(x=date, y=profit))+geom_col(color=profit)+
  scale_y_continuous(labels=comma)+
  labs(title=chart_title)
...
```{r}
chart_title <- paste0(currency,": Price vs Cost")
curr_price_cost %>% ggplot(aes(x=date, y=!!as.name(price_col)))+geom_line(color=price_line)+
  geom_line(aes(x=date, y=acb), color='black')+
  scale_y_continuous(labels=comma)+
  labs(title=chart_title)
...
```{r}
chart_title <- paste0(currency,": Net Profit")
curr_price_cost %>% ggplot(aes(x=date, y=profit))+geom_col(fill=profit)+
  scale_y_continuous(labels=comma)+
  labs(title=chart_title)
...
curr_profits %>% ggplot(aes(x=currency, y=profit))+geom_col()+
  scale_y_continuous(labels=comma)+
  labs(title=chart_title, x="")

chart_title <- "Current ROI by Currency"  
curr_profits %>% ggplot(aes(x=currency, y=roi))+geom_col()+
