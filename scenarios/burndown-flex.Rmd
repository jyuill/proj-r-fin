...
spybal <- sim_pos_yr1 %>% ggplot(aes(x=start))+geom_histogram()+
  ...
  geom_text(aes(label=prettyNum(round(mean(sim_pos_yr1$start),0), big.mark = ","), x=mean(sim_pos_yr1$start),
...
rrp <- sim_pos_all %>% ggplot(aes(x=sim, y=rrate))+geom_boxplot()+
  ...
  geom_text(aes(label=paste0(round(mean(sim_pos_all$rrate),3)*100,"%"), x=min(as.integer(sim_pos_all$sim))+1, y=mean(sim_pos_all$rrate)))+
...
ddp <- sim_pos_all %>% ggplot(aes(x=draw_ni))+geom_histogram()+
  ...
  geom_text(aes(label=prettyNum(round(mean(sim_pos_all$draw_ni),0), big.mark = ","), x=mean(sim_pos_all$draw_ni),
                ...
                y=5, hjust=1.2, vjust=0, nudge_x=1))
ggplotly(ddp)
```
