...
```{r}
h52w <- max(stock_52w$T.High)
l52w <- min(stock_52w$T.Low)
l52wa <- min(stock_52w$T.Adjusted)
cl <- last(stock_52w$T.Close, "1 day")
...
## clean up colnames by removing symbol
colnames(ds_stock52w) <- str_replace(colnames(ds_stock52w),str_glue(symb,"."), "") 
